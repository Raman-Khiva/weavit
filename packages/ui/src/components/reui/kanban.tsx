  ...
  const handleDragOver = useCallback(
    (event: DragOverEvent) => {
      ...
      if (activeContainer !== overContainer) {
        ...
        const activeContainerItems = columns[container] || []
        const activeIndex = activeContainerItems.findIndex(
          (item: T) => getItemValue(item) === active.id
        )
        const overIndex = activeContainerItems.findIndex(
          (item: T) => getItemValue(item) === over.id
        )
        ...
        if (activeIndex !== overIndex) {
          setColumns({
            ...columns,
            [container]: arrayMove(activeContainerItems, activeIndex, overIndex),
          })
        }
      }
    },
