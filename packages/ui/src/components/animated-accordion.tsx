...
const DEFAULT_ITEMS: AccordionItemData[] = [
  ...
  },
  {
    ...
    icon: <PiHandTap size={26} className="-rotate-12" />,
    content:
    ...
    <MotionConfig transition={springTransition}>
      <motion.li layout>
        <motion.div
          ...
          className="relative overflow-hidden border-b bg-background will-change-transform"
          style={{
            borderTopWidth,
            borderBottomWidth,
            borderLeftWidth,
        ...
        >
          ...
          >
            <div className="flex items-center gap-[12px] text-xs font-light">
              {item.icon}

              <span className="text-sm font-normal text-[#272729] md:text-base dark:text-zinc-100">
                {item.title}
              </span>
            </div>
          ...
          >
            <div ref={ref}>
              <div className="px-5 pb-5 text-xs font-normal text-[#545359] md:text-sm dark:text-zinc-400">
                {item.content}
              </div>
            </div>
          </motion.div>
...
export const AccordionApp: FC<AccordionProps> = ({ items }) => {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center px-2 transition-colors duration-500">
      <h2 className="global-heading mb-3 w-full">Drafts</h2>
      <ul className="w-full max-w-md min-w-xs">
